...
describe('GameCatalogComponent', () => {
  ...
  it('lists available games and links to their routes', () => {
    ...
    expect(links[2].getAttribute('href')).toBe('/games/math-cross');
    ...
    expect(images[2].getAttribute('src')).toBe('images/math-cross.png');
  });
  ...
  it('does not render unavailable placeholder games', () => {
    ...
    expect(fixture.nativeElement.querySelectorAll('.game-card')).toHaveLength(3);
